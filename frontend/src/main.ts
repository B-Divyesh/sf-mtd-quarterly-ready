...
function homePage(): string {
  return layout(`<main id="main">
    <section class="hero">
      ...
      <figure class="hero-art"><picture><source media="(max-width: 720px)" srcset="/assets/instrument-panel-720.webp"><img src="/assets/instrument-panel.webp" width="1200" height="800" fetchpriority="high" alt="A four-part quarterly dial on a paper bookkeeping machine."></picture><figcaption>Review records before preparing an accountant handoff.</figcaption></figure>
    </section>
    <section class="preview-section" aria-labelledby="preview-title"><div class="section-label"><h2 id="preview-title">Quarter status</h2></div>${previewPanel()}</section>
    <section class="how-section" aria-labelledby="how-title"><div class="section-label"><h2 id="how-title">How it works</h2></div><ol class="steps"><li><b>1</b><div><h3>Add records</h3><p>Enter a payment, attach a receipt, or import a bank CSV.</p></div></li><li><b>2</b><div><h3>Review the quarter</h3><p>Resolve each check and confirm the figures yourself.</p></div></li><li><b>3</b><div><h3>Prepare the handoff</h3><p>Download a reviewed HMRC handoff or share a CSV.</p></div></li></ol></section>
    <section class="limits-section" aria-labelledby="limits-title"><div><h2 id="limits-title">What this tool does not do</h2><p>Quarterly Ready does not run payroll or decide what you can claim.</p><p>You review every figure before exporting a compatible quarterly handoff.</p></div><div class="privacy-plate"><h3 class="plate-title">Your data</h3><p>Real records use encrypted server storage. Receipt files stay in this browser.</p><a href="/privacy" data-link>Read the privacy notice</a></div></section>
    ${pricingSection()}
  </main>`);
}
...
function recordsPage(demo: boolean): string {
  ...
  return layout(`<main id="main" class="app-main">
    ...
    <section class="control-panel" aria-labelledby="summary-title">
      <div class="dial-block"><div class="quarter-dial dial-${completion}" aria-label="${completion} of 4 checks complete"><span class="dial-hand"></span><span class="dial-centre"></span><i>1</i><i>2</i><i>3</i><i>4</i></div><div><span class="control-label">Quarter checks</span><strong>${completion} / 4 checks</strong></div></div>
    ...
    <section class="review-section" aria-labelledby="review-title"><div class="section-label"><h2 id="review-title">Review your quarter</h2></div><ol class="checklist">${checklist(doc).map((item, index) => `<li class="${item.done ? 'done' : ''}"><span>${item.done ? '✓' : index + 1}</span><div><strong>${item.title}</strong><small>${item.detail}</small></div>${item.control || ''}</li>`).join('')}</ol><div class="ready-control"><button id="mark-ready" ${completion < 4 ? 'disabled' : ''}>Mark quarter ready</button><p>${doc.markedReady ? '<strong>Quarter marked ready.</strong> You can now confirm and submit the figures yourself.' : 'Complete all four checks before marking the quarter ready.'}</p></div></section>
    <section class="handoff-section" aria-labelledby="handoff-title"><div><h2 id="handoff-title">Downloads and sharing</h2><p>Downloads stay available in the free version.</p></div><div class="output-controls"><button id="download-pack">Download accountant CSV</button><button id="share-pack" ${!demo && !isLicensed() ? 'aria-describedby="share-note"' : ''}>Make accountant link</button><button id="download-hmrc" ${sum.unresolved || !doc.figuresReviewed ? 'disabled' : ''}>Download HMRC handoff</button>${submissionControl}<p id="share-note">${demo ? 'Demo data cannot become a live accountant link or submission.' : !isLicensed() ? 'A live accountant link needs an active Sociobot subscription. The CSV and handoff remain free.' : 'Accountant links expire after 30 days.'}</p><p id="output-result" class="form-message" aria-live="polite"></p></div></section>
